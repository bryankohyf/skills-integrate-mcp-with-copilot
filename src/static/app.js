document.addEventListener("DOMContentLoaded", () => {
  ...
  // Helper to get activity type for filtering
  ...
  // Function to render activities with filters, search, and sort
  function renderActivities() {
    let entries = Object.entries(allActivities);
    // Filter
    if (filterSelect && filterSelect.value) {
      entries = entries.filter(([name]) => getActivityType(name) === filterSelect.value);
    }
    // Search
    ...
    } else if (sortSelect && sortSelect.value === "time") {
      entries.sort((a, b) => {
        ...
        return getTime(a[1].schedule).localeCompare(getTime(b[1].schedule));
      });
    }
  ...
  if (filterSelect) filterSelect.addEventListener("change", renderActivities);
